...
A_memcpy:
?OVR_memcpy:
        jmp     QWORD PTR[memcpyDispatch] # Go to appropriate version, depending on instruction set
...
.memcpySSSE3@:
    ...
    jb      .A1000              # Use simpler code if count < 64
    # count >= 64
    mov     edx, edi            # Calculate size of first block up to first regular boundary of dest
    neg     edx
    and     edx, 0x0
    jz      .B1200              # Skip if dest aligned by 16
    test    dl, 3               # edx = size of first partial block, 1 - 15 bytes
    jz      .B1030
    test    dl, 1
    jz      .B1020
    movzx   eax, BYTE PTR[rsi]  # move 1 byte
    mov     [rdi], al
    inc     rsi
    inc     rdi
.B1020:  test    dl, 2
...
.B1200:
    ...
    and     rcx,    -0x20      # Round down count to nearest multiple of 32
    add     rsi,    rcx        # Point to the end
    add     rdi,    rcx        # Point to the end
    sub     edx,    ecx        # Remaining data after loop (0-31)
    sub     rsi,    rax        # Nearest preceding aligned block of src
    # Check if count very big
    rcx, [CacheBypassLimit]
    .B1400                   # Use non-temporal store if count > CacheBypassLimit
    rcx                    # Negative index from the end
...
.align   16
.C100:
    # Code for aligned src. SSE2 and SSSE3 versions. The nice case, src and dest have same alignment.
    # Loop. rcx has negative index from the end, counting up to zero
    movaps      xmm0,   XMMWORD PTR rcx[rsi]
    movaps      xmm1,   XMMWORD PTR rcx+0x10[rsi]
    movaps      XMMWORD PTR rcx[rdi],       xmm0
    movaps      XMMWORD PTR rcx+0x10[rdi],  xmm1
    add         rcx, 0x20
    jnz         .C100
    add         rsi, rdx                    # Move the remaining edx bytes (0 - 31):
    add         rdi, rdx
    neg         rdx
    jz          .C500                       # Skip if no more data
    cmp         edx, -0x10                  # move 16-8-4-2-1 bytes, aligned
    jg          .C200
    movaps      xmm0, XMMWORD PTR rdx[rsi]  # move 16 bytes
    movaps      XMMWORD PTR rdx[rdi], xmm0
    add         rdx, 0x10
.C200:
    cmp         edx, -8
    jg          .C210        
    mov         rax, DWORD PTR rdx[rsi]     # move 8 bytes
    mov         DWORD PTR rdx[rdi], rax
    add         rdx, 8 
    jz          .C500                       # Early skip if count divisible by 8       
.C210:
    cmp         edx, -4
    jg          .C220        
    mov         eax, [rsi+rdx] # move 4 bytes
    mov         [rdi+rdx], eax
    add         rdx, 4        
.C220:   cmp     edx, -2
    jg      C230        
...
memcpyCPUDispatch:   ; CPU dispatcher, check for instruction sets and which method is fastest
        push    rbx ; This part is executed only once
        push    rcx
        push    rdx
        push    rsi
        push    rdi
        ...
        call    GetMemcpyCacheLimit@   ; set CacheBypassLimit to half the size of the largest level cache
        mov     eax, 1
