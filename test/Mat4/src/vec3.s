...
vec3_cpy:
    ...
    sal     rax,  0x20                  # sal-> signed shift left, shl-> unsigned shift left
    mov     rdx,  rax
    mov     eax,  DWORD PTR [rdi]       # load src[2] into xmm1hl
    or      rax,  rdx                   # load src[1] into xmm0hh
    mov     QWORD PTR -16[rsp], rax     # load src[0] into xmm0hl
    movq    xmm0, QWORD PTR -16[rsp]    # return xmm0, xmm1
    ret
    .cfi_endproc
/**
...
vec3_add_scalar:
    .cfi_startproc
    movd    rcx, xmm0
    pinsrd  xmm1, ecx, 0x00
    pinsrd  xmm1, ecx, 0x01
    pinsrd  xmm1, ecx, 0x02
    movaps  xmm0, [rdi]
    addps   xmm0, xmm1
    movaps  [rdi], xmm0
    mov rax, rdi
