...
static inline void
_mat4_set(mat4_t *dest, const mat4_t *src)
{
    memcpy(dest, src, SIZEOFMAT4);
}

mat4_t *mat4_set(mat4_t *dest, const mat4_t *src)
{
    _mat4_set(dest, src);
    return dest;
}
...
mat4_t *mat4_rot_from_vec(mat4_t *m, const vec3_t *v, const float degrees)
{
    ...
    const float nc = 1.f - c, xs = v->x * s, ys = v->y * s, zs = v->z * s,
        xy = v->x * v->y, xz = v->x * v->z, yz = v->y * v->z;
    const float xync = xy * nc, xznc = xz * nc, yznc = yz * nc;
...
Mat4 *setRotationMat4(Mat4 *mat, const Vec3 *v, const float degrees)
{
    ...
    mat->data[11] = mat->data[12] = mat->data[13] = mat->data[14] = 0.f;
    mat->data[15] = 1.f;
    return mat;
}
...
mat4_t *mat4_mul(mat4_t *dest, mat4_t *src)
{
    mat4_t m;
    _mat4_set(&m, dest);

    dest->data[ 0] = m.data[0] * src->data[ 0] + m.data[ 4] * src->data[ 1] +
                     m.data[8] * src->data[ 2] + m.data[12] * src->data[ 3];
    dest->data[ 4] = m.data[0] * src->data[ 4] + m.data[ 4] * src->data[ 5] +
                     m.data[8] * src->data[ 6] + m.data[12] * src->data[ 7];
    dest->data[ 8] = m.data[0] * src->data[ 8] + m.data[ 4] * src->data[ 9] +
                     m.data[8] * src->data[10] + m.data[12] * src->data[11];
    dest->data[12] = m.data[0] * src->data[12] + m.data[ 4] * src->data[13] +
                     m.data[8] * src->data[14] + m.data[12] * src->data[15];
    dest->data[ 1] = m.data[1] * src->data[ 0] + m.data[ 5] * src->data[ 1] +
                     m.data[9] * src->data[ 2] + m.data[13] * src->data[ 3];
    dest->data[ 5] = m.data[1] * src->data[ 4] + m.data[ 5] * src->data[ 5] +
                     m.data[9] * src->data[ 6] + m.data[13] * src->data[ 7];
    dest->data[ 9] = m.data[1] * src->data[ 8] + m.data[ 5] * src->data[ 9] +
                     m.data[9] * src->data[10] + m.data[13] * src->data[11];
    dest->data[13] = m.data[1] * src->data[12] + m.data[ 5] * src->data[13] +
                     m.data[9] * src->data[14] + m.data[13] * src->data[15];
    dest->data[ 2] = m.data[ 2] * src->data[ 0] + m.data[ 6] * src->data[ 1] +
                     m.data[10] * src->data[ 2] + m.data[14] * src->data[ 3];
    dest->data[ 6] = m.data[ 2] * src->data[ 4] + m.data[ 6] * src->data[ 5] +
                     m.data[10] * src->data[ 6] + m.data[14] * src->data[ 7];
    dest->data[10] = m.data[ 2] * src->data[ 8] + m.data[ 6] * src->data[ 9] +
                     m.data[10] * src->data[10] + m.data[14] * src->data[11];
    dest->data[14] = m.data[ 2] * src->data[12] + m.data[ 6] * src->data[13] +
                     m.data[10] * src->data[14] + m.data[14] * src->data[15];
    dest->data[ 3] = m.data[ 3] * src->data[ 0] + m.data[ 7] * src->data[ 1] +
                     m.data[11] * src->data[ 2] + m.data[15] * src->data[ 3];
    dest->data[ 7] = m.data[ 3] * src->data[ 4] + m.data[ 7] * src->data[ 5] +
                     m.data[11] * src->data[ 6] + m.data[15] * src->data[ 7];
    dest->data[11] = m.data[ 3] * src->data[ 8] + m.data[ 7] * src->data[ 9] +
                     m.data[11] * src->data[10] + m.data[15] * src->data[11];
    dest->data[15] = m.data[ 3] * src->data[12] + m.data[ 7] * src->data[13] +
                     m.data[11] * src->data[14] + m.data[15] * src->data[15];
    return dest;
    /*
    dest->data[ 0] = m.data[0] * src->data[ 0] + m.data[ 4] * src->data[ 1] +
                     m.data[8] * src->data[ 2] + m.data[12] * src->data[ 3];
...
Mat4 *mulMat4(Mat4 *dest, Mat4 *src)
{
    ...
    for (i = 0; i < 4; ++i) {
        ...
        dest->data[i]       = c[0] * r[ 0] + c[1] * r[ 1] +
                              c[2] * r[ 2] + c[3] * r[ 3];
        dest->data[i + 0x4] = c[0] * r[ 4] + c[1] * r[ 5] +
                              c[2] * r[ 6] + c[3] * r[ 7];

        dest->data[i + 0x8] = c[0] * r[ 8] + c[1] * r[ 9] +
                              c[2] * r[10] + c[3] * r[11];
        dest->data[i + 0xc] = c[0] * r[12] + c[1] * r[13] +
                              c[2] * r[14] + c[3] * r[15];
    }
...
Mat4 *rotateMat4(Mat4 *mat, const Vec3 *v, const float degrees)
{
    ...
    return mat4_mul(mat, setRotationMat4(&tmp, v, degrees));
...
{
    Mat4 tmp;
    return mat4_mul(mat, setRotXMat4(setMat4(&tmp, mat), degrees));
...
{
    Mat4 tmp;
    return mat4_mul(mat, setRotYMat4(setMat4(&tmp, mat), degrees));
...
{
    Mat4 tmp;
    return mat4_mul(mat, setRotZMat4(setMat4(&tmp, mat), degrees));
...
Mat4 *setLookAtMat4(Mat4 *mat, Vec3 *pos, Vec3 *tar, Vec3 *up)
{
    ...
    mat->data[11] = mat->data[12] = mat->data[13] = mat->data[14] = 0.0f;
    mat->data[15] = 1.0f;
    return mat;
}
