 ...
 * Created by nimitz in 2015-Mar-11
...
vec3 maptex(vec3 p)
{
     p.z -= hash(floor(p.x/c+1.))*(uTime*12.+92.);
    p.y -= hash(floor(p.z/c+1.))*(uTime*3.+89.);
    vec3 iq = floor(p);
    p = fract(p)-0.5;
    p.xz *= mm2(uTime*2.+iq.x);
    p.xy *= mm2(uTime*0.6+iq.y);
    return p;
}
...
float map(vec3 p)
{
    ...
    q.xz *= mm2(uTime*2.+iq.x);
    q.xy *= mm2(uTime*0.6+iq.y);
...
vec3 bg(in vec3 d)
{
    return abs(sin(vec3(1.,2.,2.5)+sin(uTime*0.05)))*0.4+.35*(cells(d*.5)*0.4+0.6);
}
...
void main()
{
    ...
    vec3 ro = vec3(0,0,-uTime*20.);
    rd = normalize(vec3(p,-1.));
    rd = rotx(rd,mo.y+sin(uTime*0.4)*0.5);
    rd = roty(rd,-mo.x+cos(uTime)*0.1);
    ...
    vec3 pos = ro + rd*FAR;
    vec3 lpos = ro + vec3(0,sin(uTime*1.)*2., -15.+sin(uTime*0.5)*10.);

    if ( rz < FAR )
    {
        pos = ro+rz*rd;
        float d= distance(ro,pos);
        color = shade(pos, rd,lpos, d);
    }

    color = mix(color, bgc, smoothstep(FAR-40.,FAR,rz));
    color += flare(ro,rd,rz,lpos,1.);

    //Post
    #if 1
    color = clamp(color,0.,1.)*1.3;
    color -= hash(color.xy+p.xy)*.017; //Noise
    color -= smoothstep(0.15,1.9,length(p*vec2(1,1.5)))*.6; //Vign
    color = pow(clamp(color,0.,1.),vec3(0.8));
    color =cc(color,.6,.3); //Color modifier
    #endif
